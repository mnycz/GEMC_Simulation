...
{	
	...
	$detector{"style"}       = 1;
#    $detector{"sensitivity"}  = "rtpc"; ## HitProcess definition
#    $detector{"hit_type"}     = "rtpc"; ## HitProcess definition #I changed these 11/23
	print_det(\%configuration, \%detector);
}
...
sub make_readout_layer
{
	...
	$detector{"material"}    = $mate;
	$detector{"style"}       = 1;
    $detector{"sensitivity"}  = "rtpc"; ## HitProcess definition #I changed these
    $detector{"hit_type"}     = "rtpc"; ## HitProcess definition 
	print_det(\%configuration, \%detector);
}
...
sub make_dipole_field
{
    my %detector = init_det();
    $detector{"name"} = "dfield";
    $detector{"mother"}      = "rtpc";
    $detector{"description"} = "dipole";
    $detector{"pos"}         = "0*cm 0*cm 0*cm";
    $detector{"color"}       = "33bb99";
    $detector{"type"}        = "Box";
    $detector{"dimensions"}  = "6.0*cm 6.0*cm 23.0*cm";
    $detector{"material"}    = "G4_Galactic";
    $detector{"visible"}     = 1;
    $detector{"mfield"}      = "dipoleField";
    $detector{"style"}       = 1;
    print_det(\%configuration, \%detector);
...
sub make_Collimator
{
    #my $collimator_zpos     = -30.0; # cm
    #my $collimator_zpos     = -25.5; # cm
    #my $collimator_zpos     = -35.0; # cm
    my $collimator_zpos     = -25.0; # cm 
    my %detector = init_det();
    $detector{"name"}        = "collimator";
    $detector{"mother"}      = "root";
    $detector{"pos"}         = "0*cm 0.0*cm $collimator_zpos*cm";
    $detector{"color"}       = "33bb99";
    #$detector{"type"}        = "Cons";
    $detector{"type"}        = "Tube"; 
    #$detector{"dimensions"}  =  "8*mm 10*mm 4*mm 6*mm 1.5*cm 0.*deg 270.*deg";
    $detector{"dimensions"}    ="10*mm 8.0*cm 3.0*cm 0*deg 360*deg"; #inner_radius, outer_radius, half-length, starting_angle, total angle
...
sub make_Collimator_2
{
    ...
    $detector{"mother"}      = "root";
    $detector{"type"}        = "Tube";
    $detector{"dimensions"}    = "0.*cm 8.0*cm 5.5*cm 0*deg 360*deg"; #inner_radius, outer_radius, half-length, starting_angle, total angle
    $detector{"material"}    = "Component";
    print_det(\%configuration, \%detector);
    ...
    $detector{"mother"}      = "root";
    $detector{"type"}        = "Cons";
    $detector{"dimensions"}  =  "0*mm 0.06*mm 0*mm 10mm 5.55*cm 0.*deg 360.*deg";    
    $detector{"material"}    = "Component";
    print_det(\%configuration, \%detector);
     ...
     $detector{"name"}        = "cone2";
     $detector{"mother"}      = "root";
     $detector{"pos"}         = "0*cm 0.0*cm -27.0*cm";
...
sub make_Radiator
{
    #my $radiator_zpos     = -33; # cm --- good number for just collimator    
    my %detector = init_det();
    $detector{"name"}        = "radiator";
    $detector{"mother"}      = "root";
    $detector{"description"} = "Cu radiator";
    $detector{"pos"}         = "0*cm 0*cm -36*cm";
    #$detector{"pos"}         = "0*cm 0*cm -35*cm";
    $detector{"color"}       = "33bb99";
    $detector{"type"}         = "Box";
    #$detector{"dimensions"}  = "1*cm 1*cm 0.05*cm";
    $detector{"dimensions"}  = "1.0*cm 1.0*cm 0.065*cm"; #9%  
...
make_usep(); #don't make upstream end plate 11/25
